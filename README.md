...
## Introduction
This project acts as an early intersection of my dual studies in computer science and film/media. My task for this project was to create a Python-based command-line interface (CLI) program capable of parsing, filtering, and analyzing a text-based database of movies. With the data properly loaded into parallel lists, the program executes various search queries and can export a sorted version of the database using a custom selection sort algorithm.
...
&emsp; ***Data Computation***

* The `avgFilmRuntimeByRating()` function calculates the mathematical average of runtimes for all films that share a specific MPAA rating[cite: 6].
...
## How To Run
If you want to run this program, you must download both the `breene_z_movies.py` script and the `movies.txt` database file (or provide your own properly formatted comma-separated text file). Ensure both files are located in the same directory. Run the program using a Python 3 interpreter via your terminal or IDE (e.g., typing `python breene_z_movies.py`). When prompted for the file name at launch, type `movies.txt`. From there, you can interact with the numbered CLI menu by typing an integer (1-7) to execute the various search, sort, and computation functions.
